...
**SPEC-01.6 — Indicador visual de modo lectura**

### Requirement: Indicador visual de modo lectura para visitantes
Las páginas de programa (Entre Semana y Fin de Semana) SHALL mostrar un indicador visual de "Solo lectura" cuando el usuario no tiene rol `admin`, para comunicar explícitamente el modo de la interfaz. Este requisito aplica también a usuarios **anónimos** (no autenticados), no solo a visitantes autenticados sin rol admin.
...
## SPEC-11 — Tipografía y Diseño Visual
...
- **THEN** el EmptyState muestra mensaje descriptivo del contexto sin CTA (el usuario no puede auto-asignarse)
